...
def compute_period(data, datestr, magstr, magerrstr):
  epoch, mag, mag_err = [data[datestr].values,data[magstr].values,data[magerrstr].values]
  NN = 2000
  ...
  for ii in range(NN):
    loc_new = np.random.choice(len(mag), replace=False, size=int(len(mag)*0.4))
    epoch_new = epoch[loc_new]
    mag_new = mag[loc_new]
    mag_err_new = mag_err[loc_new]

    freq, power = LombScargle(epoch_new, mag_new, mag_err_new, fit_mean=True).autopower(minimum_frequency=1, maximum_frequency=5)
  ...
  possible_periods, counts = np.unique(np.sort(np.round(period_array,5)), return_counts=True)
...
def compute_lombscargle_period(seed, epoch, mag, mag_err):
    np.random.seed(seed)
    loc_new = np.random.choice(len(mag), size=int(len(mag) * 0.4), replace=False)
    epoch_new = epoch[loc_new]
    mag_new = mag[loc_new]
    mag_err_new = mag_err[loc_new]

    freq, power = LombScargle(epoch_new, mag_new, mag_err_new,  fit_mean=True).autopower(minimum_frequency=1.1, maximum_frequency=4)
    best_freq = freq[np.argmax(power)]
    period = 1 / best_freq
    return period
...
def compute_period_nb(data, datestr, magstr, magerrstr, seed=42):
    ...
    with mp.Pool(mp.cpu_count(), initializer=initialize_worker, initargs=(seed,)) as pool:
        results = pool.starmap(compute_lombscargle_period, [(s, epoch, mag, mag_err) for s in seeds])
