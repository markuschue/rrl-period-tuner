...
from utils.data_utils import (combine_photometry_data, get_mag_field_names,
                              get_star_photometry, parse_gaia_time)
...
class PhaseFinder():
    def __init__(self, star_id: str, photometry_path: str):
        ...
        self.period = compute_period(
            self.combined_data, self.datestr, self.magstr, self.magerrstr)[0]

        # Reference time will be the time where the magnitude is minimum
        self.reference_time = parse_gaia_time(
            self.combined_data.iloc[self.combined_data[self.magstr].idxmin(
            )][self.datestr]
        ).to_datetime()
